...
func (h HeaderVisitor) parseFieldTag(tag_data string) (string, int, int, bool, bool) {
	...
	var values []string
	values, present = h.header[field_name]
...
func (h HeaderVisitor) Visit(field reflect.Value, tag_data string, debug bool) error {
	key, start, end, omitempty, present := h.parseFieldTag(tag_data)
	if !present {
		if debug {
	...
	if start == 0 && end == -1 {
		s_value = h.header.Get(key)
		a_value = h.header[key]
	} else if (start - end) == 0 {
		s_value = h.header[key][start]
		a_value = []string{h.header[key][start]}
	} else if (start - end) == 1 {
		s_value = h.header[key][start]
		a_value = []string{h.header[key][start]}
	}
